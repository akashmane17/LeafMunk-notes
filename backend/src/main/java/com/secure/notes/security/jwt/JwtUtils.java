...
public class JwtUtils {
    ...
    public Cookie createJwtCookie(String jwtToken) {
        ...
        // Set cookie to expire after 7 days (adjustable)
        cookie.setMaxAge(7 * 24 * 60 * 60);  // 7 days in seconds

        // Set path to root
        cookie.setPath("/");

        // Set HttpOnly to true so that it's not accessible via JavaScript (for security)
        cookie.setHttpOnly(true);

        // Set Secure flag to true if in production (adjust as per your environment)
//        if (isProductionEnvironment()) {
//            cookie.setSecure(true);  // Use Secure flag only in HTTPS
//        }
